...
def setup_supabase_connection(**context):
    ...
    # Get credentials from environment with more robust handling
    print("🔍 Reading environment variables...")
    
    # Try multiple ways to get environment variables
    def get_env_var(var_name, default=''):
        """Get environment variable with multiple fallback attempts"""
        ...
        if not value:
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    print(f"📋 Found {var_name} via subprocess: {'✅' if value else '❌'}")
            except Exception as e:
                print(f"⚠️ Subprocess method failed for {var_name}: {e}")
        return value
    ...
    db_port = get_env_var('DB_PORT', '6543')
    db_name = get_env_var('DB_NAME', 'postgres')
    
    print("🔍 Environment variables status:")
    print(f"  - DB_USER: {'✅ Set' if db_user else '❌ Missing'} - Length: {len(db_user) if db_user else 0}")
    print(f"  - DB_PASSWORD: {'✅ Set' if db_password else '❌ Missing'} - Length: {len(db_password) if db_password else 0}")
    print(f"  - DB_HOST: {'✅ Set' if db_host else '❌ Missing'} - Value: '{db_host}'")
    print(f"  - DB_PORT: {db_port}")
    print(f"  - DB_NAME: {db_name}")
    
    # Debug: Print raw environment variable values (be careful with password)
    print("🔍 Raw environment variable check:")
    raw_vars = ['DB_USER', 'DB_HOST', 'DB_PORT', 'DB_NAME']
    for var in raw_vars:
        raw_value = os.environ.get(var, 'NOT_SET')
        print(f"  - {var}: '{raw_value}'")
    
    # Check password separately (don't print value)
    raw_password = os.environ.get('DB_PASSWORD', 'NOT_SET')
    print(f"  - DB_PASSWORD: {'SET' if raw_password != 'NOT_SET' else 'NOT_SET'} (length: {len(raw_password) if raw_password != 'NOT_SET' else 0})")
    ...
    if not all([db_user, db_password, db_host]):
        ...
        print(f"❌ CRITICAL: Missing Supabase credentials: {missing_vars}")
        print("📋 Current detailed status:")
        print(f"  - DB_USER: '{db_user}' (empty: {not db_user})")
        print(f"  - DB_PASSWORD: {'***' if db_password else 'EMPTY'} (empty: {not db_password})")
        print(f"  - DB_HOST: '{db_host}' (empty: {not db_host})")
        print("")
        print("🔧 Troubleshooting steps:")
        print("1. Verify .env file exists and has correct format:")
        print("   cat .env | grep DB_")
        print("")
        print("2. Check if Docker is loading environment variables:")
        print("   docker compose exec airflow-standalone env | grep DB_")
        print("")
        print("3. Verify docker-compose.yml env_file setting:")
        print("   grep -A 5 env_file docker-compose.yml")
        print("")
        print("4. Restart containers to reload environment:")
        print("   docker compose down && docker compose up")
        print("")
        print("5. Alternative: Set variables directly in docker-compose.yml environment section")
        print("")
        ...
        context['task_instance'].xcom_push(key='missing_credentials', value=missing_vars)
        
        # Don't raise exception - let DAG continue in limited mode
    ...
    try:
        ...
        context['task_instance'].xcom_push(key='supabase_connection_status', value='success')
        ...
        # Additional validation - test direct connection if psycopg2 is available
        try:
            import psycopg2
            ...
            connection_details['direct_test'] = 'success'
        except ImportError:
            print("⚠️ psycopg2 not available for direct connection test")
            connection_details['direct_test'] = 'psycopg2_unavailable'
        except Exception as e:
            print(f"❌ Direct connection test failed: {str(e)}")
            connection_details['direct_test'] = f'failed: {str(e)}'
        
        # Update XCom with final connection details
        context['task_instance'].xcom_push(key='supabase_connection_details', value=connection_details)
        ...
        # Store error details in XCom
        context['task_instance'].xcom_push(key='supabase_connection_status', value='failed_setup_error')
        context['task_instance'].xcom_push(key='supabase_connection_error', value=str(e))
        
        # Don't raise the exception - let DAG continue in limited mode
        print("⚠️ Setup failed but DAG will continue in limited mode")
        return
        
    print("🎉 Supabase connection setup completed successfully!")
