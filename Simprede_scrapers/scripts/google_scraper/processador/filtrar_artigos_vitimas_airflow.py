...
def find_input_file(project_root, target_date):
    ...
    checking the year/month/day organized structure with multiple date formats
    """
    ...
    # Look for the output from processar_relevantes_airflow with multiple date formats
    possible_paths = [
        # PRIORITY: Date-specific organized structure from processar_relevantes_airflow (structured data)
        ...
        # PRIORITY: Date-specific organized structure from processar_relevantes_airflow (raw data) 
        ...
        # Alternative formats that might be created by processar_relevantes
        os.path.join(project_root, "data", "structured", current_year, current_month, current_day, f"artigos_google_municipios_pt_{current_date}.csv"),
        ...
        # Check for files without date suffix in organized structure
        os.path.join(project_root, "data", "structured", current_year, current_month, current_day, "artigos_google_municipios_pt.csv"),
        os.path.join(project_root, "data", "raw", current_year, current_month, current_day, "artigos_google_municipios_pt.csv"),
        
        # Legacy format with date suffix in structured directory
        os.path.join(project_root, "data", "structured", f"artigos_google_municipios_pt_{date_suffix}.csv"),
        os.path.join(project_root, "data", "structured", f"artigos_google_municipios_pt_{current_date}.csv"),
        
        # Fallback to structured directory without date
        ...
        os.path.join(project_root, "data", "raw", f"artigos_google_municipios_pt_{date_suffix}.csv"),
        os.path.join(project_root, "data", "raw", "artigos_google_municipios_pt.csv")
    ]
    
    log_progress(f"🔍 Searching for input file with target date: {date_suffix} ({current_date})")
    log_progress(f"📁 Project root: {project_root}")
    
    for i, path in enumerate(possible_paths):
        log_progress(f"   {i+1:2d}. Checking: {path}")
        if os.path.exists(path):
            file_size = os.path.getsize(path)
            log_progress(f"✅ Found input file: {path} ({file_size} bytes)")
            return path
    
    log_progress(f"❌ No input file found. Searched {len(possible_paths)} possible locations:", "error")
    log_progress(f"   Expected formats:", "error")
    log_progress(f"   - artigos_google_municipios_pt_{date_suffix}.csv", "error")
    log_progress(f"   - artigos_google_municipios_pt_{current_date}.csv", "error")
    log_progress(f"   - artigos_google_municipios_pt.csv", "error")
...
def setup_paths_and_dates(target_date=None, dias=1, input_file=None, output_dir=None, date_str=None):
    ...
    # Handle different date string formats from DAG
    if date_str:
        if len(date_str) == 8 and date_str.isdigit():  # Format: 20250614
            formatted_date_str = f"{date_str[:4]}-{date_str[4:6]}-{date_str[6:8]}"
        else:
            formatted_date_str = date_str  # Already in YYYY-MM-DD format
    else:
    ...
    log_progress(f"   Project root: {PROJECT_ROOT}")
    log_progress(f"   Target date: {dt.strftime('%Y-%m-%d')}")
    log_progress(f"   Date string: {date_str} -> {formatted_date_str}")
    ...
    OUTPUT_CSV = os.path.join(PROCESSED_DATA_DIR, f"artigos_vitimas_filtrados_{current_date}.csv")
    
    # Additional output files for better organization
    NO_VICTIMS_CSV = os.path.join(PROCESSED_DATA_DIR, f"artigos_sem_vitimas_{current_date}.csv")
