...
def main():
    ...
    original_mention_count = len(all_mentions)
    ...
    print(f"\n🔍 Aplicando filtro de falsos positivos sobre {len(all_mentions)} menções...")
    all_mentions = filter_false_positives(all_mentions)
    filtered_count = original_mention_count - len(all_mentions)
    print(f"   ↳ {filtered_count} falsos positivos removidos ({filtered_count/original_mention_count*100:.1f}% do total).")
    
    if not all_mentions:
        print("⚠️ Nenhuma menção válida de vítima após filtragem.")
        return
    ...
    print(f"✅ Total de menções extraídas após filtragem: {len(all_mentions)}")
    ...
    print(f"\n📈 Média de {len(all_mentions)/max(1, total_articles_with_mentions):.2f} menções por artigo com vítimas")
